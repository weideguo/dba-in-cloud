...
from libs.tc import MongodbOpt as DbOpt
...
def create_backup_download_task(backup_date, instance, filename, max_try=5):
    """
    获取实例信息并创建下载任务
    """
    i=0
    ...
    for region, region_instance_list in all_instances:    
        for l in region_instance_list:            
            ...
            try:
                ...
                backup_name = backup_info.BackupName
                backup_time = backup_info.StartTime
                if backup_name:                    
                    r = do.create_backup_download_task(region, instance_id, backup_name)   
                    if not r.Tasks:
                        raise Exception("create_backup_download_task error %s %s %s %s" % (region, instance_id, backup_name, str(r))) 
                    
                    _filename = filename if filename else "%s_%s.tar" % (instance_name, backup_time.replace("-","").replace(" ","").replace(":","") )
                    download_url = None
                    timeout = 1800
                    init_info = {"url":download_url, "path":config.mongodb_backup_dir, "filename":_filename, "download_timeout":timeout,\
                                 "region":region, "instance_id":instance_id, "backup_name":backup_name}
                    retry_queue.put(init_info)
                    
                else:
                    raise Exception("null for backup_name")           
...
def set_download_queue(timeout = 600):
    ...
    while True:
        try:
            ...
            backup_name = retry_info["backup_name"]
            ...
            try:
                download_url = do.describe_backup_download_task(region, instance_id, backup_name)
            except:
                logger_err.error(format_exc())
                download_url = None
...
def main(backup_date, instance, filename):
    ...
    create_backup_download_task(backup_date, instance, filename)         # 创建备份下载任务
